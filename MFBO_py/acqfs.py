...
def runMes(model, bounds, fidelity_history, previous_evaluations=None, train_x_past=None):
    ...
    candidate_set_no_hf = min_vals + (max_vals - min_vals) * torch.rand(1000, bounds.shape[1])#originally 10000
    candidate_set = torch.tensor(np.concatenate((candidate_set_no_hf, np.array([[random.choice(fidelities) for x in range(1000)]]).T), axis=1))#originally 10000
    ...
    #cost_model = AffineFidelityCostModel(fidelity_weights=target_fidelities, fixed_cost=1.0) #fixed cost potentially needs reviewing 
    cost_model = AffineFidelityCostModel(fidelity_weights=target_fidelities, fixed_cost=0)
...
def runEI(model, bounds, fidelity_history, previous_evaluations, train_x_past=None): 
    ...
    candidate_set_ei= min_vals + (max_vals - min_vals) * torch.rand(500, bounds.shape[1]) #originally down to 5000 from 10000
...
def runEI_lf(model, bounds, fidelity_history, previous_evaluations, train_x_past=None): 
    
    min_vals = bounds[0]  
    max_vals = bounds[1]

    candidate_set_ei= min_vals + (max_vals - min_vals) * torch.rand(500, bounds.shape[1]) #originally down to 5000 from 10000
    lf_column = torch.full((candidate_set_ei.shape[0], 1),0.01, dtype=torch.float32) #change based on lf_cost value!
    candidate_set = torch.cat((candidate_set_ei, lf_column), dim=1)
...
def runSF_MES(model, bounds, fidelity_history, previous_evaluations, train_x_past=None): 
    
    min_vals = bounds[0]  
    max_vals = bounds[1]

    candidate_set_sfmes= min_vals + (max_vals - min_vals) * torch.rand(500, bounds.shape[1]) #originally down to 5000 from 10000
    ones_column = torch.ones((candidate_set_sfmes.shape[0], 1), dtype=torch.float32)
    candidate_set = torch.cat((candidate_set_sfmes, ones_column), dim=1)

    
    qMES_acqf = qMaxValueEntropy(model, 
                            candidate_set)
    
    acquisitionScores =  qMES_acqf.forward(candidate_set.reshape(-1,1, bounds.shape[1]+1) ).detach()
